...
function SobrePeso($data): int {
    $cont = 0;
    foreach ($data as $value){
        if (EdadHumana($value["edad"]) >= 60 && $value["peso"] >= 7){
        $cont++;
    } elseif (EdadHumana($value["edad"]) <= 40 && $value["peso"] >= 3) {
        $cont++;
    } 
}
    return $cont;
...
$resultado = SobrePeso($data);
$porcentaje = $resultado * count($data) / 100;

echo "$resultado con sobrepeso <br> ";
echo count($data), " totales <br> ";



echo " El porcentaje es del $porcentaje %";
